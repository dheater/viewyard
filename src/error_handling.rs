...
pub fn handle_checkout_error(branch_name: &str, repo_path: &std::path::Path, stderr: &str) -> Result<()> {
...
pub fn handle_branch_creation_error(branch_name: &str, repo_path: &std::path::Path, stderr: &str) -> Result<()> {
...
fn show_repo_not_found_error(repo_name: &str) -> Result<()> {
    ...
    ui::print_info(&format!("   • Verify repository exists: gh repo view {repo_name}"));
...
fn show_directory_exists_error(repo_name: &str) -> Result<()> {
    ...
    ui::print_info(&format!("   • Remove existing directory: rm -rf {repo_name}"));
...
fn show_uncommitted_changes_error(branch_name: &str, repo_path: &std::path::Path) -> Result<()> {
    ui::print_error(&format!("Cannot checkout branch '{branch_name}' - uncommitted changes"));
...
fn show_generic_checkout_error(branch_name: &str, repo_path: &std::path::Path, stderr: &str) -> Result<()> {
    ui::print_error(&format!("Failed to checkout branch '{branch_name}'"));
    ui::print_info("Branch checkout failed:");
    ui::print_info(&format!("   • Error: {}", stderr.trim()));
    ui::print_info(&format!("   • Check branch status: cd {} && git status", repo_path.display()));
...
fn show_branch_exists_error(branch_name: &str) -> Result<()> {
    ...
    ui::print_info(&format!("   • Use existing branch: git checkout {branch_name}"));
    ui::print_info(&format!("   • Or delete existing: git branch -D {branch_name}"));
    ui::print_info("   • Then retry view creation");
    anyhow::bail!("Branch '{branch_name}' already exists")
}

fn show_generic_branch_creation_error(branch_name: &str, repo_path: &std::path::Path, stderr: &str) -> Result<()> {
    ui::print_error(&format!("Failed to create branch '{branch_name}'"));
    ui::print_info("Branch creation failed:");
    ui::print_info(&format!("   • Error: {}", stderr.trim()));
    ui::print_info(&format!("   • Check repository state: cd {} && git status", repo_path.display()));
